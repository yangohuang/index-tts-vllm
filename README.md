...
- **TTFA**（Time To First Audio，首音频延迟）：从发出请求到客户端收到第一段可播放音频的耗时。对话/数字人场景的核心体验指标——它决定"说完话后要沉默多久才听到回应"。
- **RTF**（Real-Time Factor，实时率）：合成耗时 ÷ 音频时长。RTF = 0.25 表示合成 10 秒音频只需 2.5 秒；小于 1 即可边合成边播放不卡顿，数值越小、单卡能同时服务的路数越多。
- **冷缓存 / 热缓存**：每次合成都需要先从参考音频提取说话人特征（音色向量、语义特征等），这一步约需 250 ms。本 Fork 会把提取结果按文件缓存——**某个参考音频第一次被使用**时需要现算（冷缓存），**之后再用同一个参考音频**就直接命中缓存（热缓存）。线上业务的音色通常固定，所以热缓存才是常态。

测试条件：单卡 RTX 4090，约 130 字中文文本（合成音频约 24 秒），每组丢弃 1 次预热后取 3 次测量的中位数。下表从上到下是优化的叠加过程，**最后一行是当前默认配置**：

| 模式 | TTFA | RTF | 说明 |
| --- | --- | --- | --- |
| 非流式 `/tts_url`（原始方式） | ~4020 ms | 0.17 | 整句合成完才返回，客户端要干等 4 秒 |
| 流式，冷缓存 | ~830 ms | 0.31 | 改为边合成边推流；首次使用该参考音频，含 ~250 ms 特征提取 |
| 流式，热缓存 | ~600 ms | 0.31 | 说话人特征命中缓存，省掉提取耗时 |
| 流式，热缓存 + 首块 10 步扩散 | ~467→365 ms | 0.31 | 只把**第一块**音频的扩散步数从 25 降到 10（试听无差异），后续块不受影响 |
| **流式 + 增量解码（当前默认）** | **~365 ms** | **0.25** | 已合成部分不再重复解码，合成速度再提升 ~10%，音质经同 token 对比与试听验证无损 |
